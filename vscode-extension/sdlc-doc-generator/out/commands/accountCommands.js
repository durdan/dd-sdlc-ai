...
exports.AccountCommands = void 0;
...
class AccountCommands {
    ...
    async checkUsage() {
        ...
        const report = [
            ...
            '',
            `**Recent Documents**`,
            ...usage.documents.slice(-5).reverse().map(doc => `• ${this.getDocumentTypeName(doc.type)} - ${new Date(doc.timestamp).toLocaleTimeString()}`)
        ].join('\n');
        // Show usage panel
        const panel = vscode.window.createWebviewPanel('sdlc-usage', 'SDLC Usage Report', vscode.ViewColumn.One, {
            enableScripts: true
        });
    ...
    getUsageWebviewContent(report, usage, limit, isAuthenticated) {
...
exports.AccountCommands = AccountCommands;
